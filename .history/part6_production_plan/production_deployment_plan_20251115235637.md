...
**Key Decisions:**
- **Architecture:** Serverless-first on AWS (Lambda + RDS + S3)
- **Estimated Monthly Cost:** $200-500 for MVP, scaling to $1,500-2,500 at 100 clients
- **Timeline:** 6-8 weeks from kickoff to production
- **Primary Risk:** Data pipeline reliability and Facebook API rate limits
...
┌─────────────────────────────────────────────────────────────────┐
│                        DATA SOURCES                              │
│  Facebook Ads API │ CRM System │ Internal Database │ Manual CSV │
└────────────────┬────────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────────┐
│                    DATA INGESTION LAYER                          │
│  AWS Lambda (Python) - Scheduled ETL Jobs                        │
│  - Facebook API connector (every 4 hours)                        │
│  - CRM webhook listener (real-time)                              │
│  - CSV file processor (S3 trigger)                               │
└────────────────┬────────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────────┐
│                       DATA STORAGE                               │
│  AWS RDS PostgreSQL (primary)  │  AWS S3 (raw data + archives)  │
│  - Raw data tables              │  - Daily backups                │
│  - Processed metrics            │  - Historical snapshots         │
│  - Campaign scores & history    │  - Audit logs                   │
└────────────────┬────────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────────┐
│                    SCORING ENGINE (CORE)                         │
│  AWS Lambda (Python 3.11) - Triggered every 6 hours             │
│  - Load campaign data from RDS                                   │
│  - Calculate 5-dimension scores                                  │
│  - Generate recommendations                                      │
│  - Store results + historical trends                             │
...
│  ┌─────────────┐  ┌──────────────┐  ┌────────────────────────┐ │
│  │ Email       │  │ Dashboard    │  │ API                    │ │
│  │ Alerts      │  │ (Streamlit/  │  │ (API Gateway +         │ │
│  │ (SES +      │  │ FastAPI)     │  │ Lambda)                │ │
│  │ EventBridge)│  │ on ECS/      │  │ - GET /scores          │ │
│  │             │  │ Fargate      │  │ - GET /recommendations │ │
│  └─────────────┘  └──────────────┘  └────────────────────────┘ │
└─────────────────────────────────────────────────────────────────┘
                 │
                 ▼
...
│  CloudWatch (logs, metrics) │ Sentry (errors) │ PagerDuty       │
└─────────────────────────────────────────────────────────────────┘
```
...
| **Compute** | AWS Lambda (2GB RAM, 5min timeout) | Cost-effective for scheduled batch jobs; auto-scaling |
| **Database** | AWS RDS PostgreSQL 15 (db.t4g.micro) | Relational data with complex queries; proven reliability |
| **Cache** | Redis (ElastiCache) - Optional Phase 2 | Sub-100ms API response times if needed |
| **Storage** | AWS S3 Standard-IA | Long-term storage of raw data + archives |

#### Data Ingestion
| Source | Method | Frequency |
|--------|--------|-----------|
| Facebook Ads API | AWS Lambda → RDS | Every 4 hours (6x/day) |
| CRM Webhooks | API Gateway → Lambda → RDS | Real-time (event-driven) |
| Manual CSV Uploads | S3 Event → Lambda → RDS | On-demand |

#### Delivery Layer
| Component | Technology | Rationale |
|-----------|-----------|-----------|
| **Email Alerts** | AWS SES + Jinja2 templates | $0.10 per 1,000 emails; native AWS integration |
| **Dashboard** | Streamlit on ECS Fargate | Rapid prototyping; Python-native; low maintenance |
| **API** | FastAPI + API Gateway + Lambda | Modern Python framework; auto-generated docs |
| **Authentication** | AWS Cognito | Managed auth; integrates with SES/API Gateway |

#### DevOps & Monitoring
| Component | Technology | Rationale |
|-----------|-----------|-----------|
| **IaC** | Terraform | Version-controlled infrastructure; multi-cloud ready |
| **CI/CD** | GitHub Actions | Free for private repos; native git integration |
| **Logging** | CloudWatch Logs + Insights | Native AWS; queryable with SQL-like syntax |
| **Error Tracking** | Sentry (Developer plan - $26/mo) | Real-time error alerts with full stack traces |
| **Alerting** | CloudWatch Alarms → SNS → Email/Slack | Native AWS; customizable thresholds |
| **Secrets** | AWS Secrets Manager | Auto-rotate API keys; encrypted at rest |

---

## 2. Deployment Strategy

### 2.1 Phased Rollout (6-8 Weeks)

#### **Phase 1: MVP Backend (Weeks 1-3)**
**Goal:** Core scoring engine operational with manual triggers
...
2. ✅ Scoring Lambda function deployed
   - Containerized Python app (Docker)
   - Manual invoke via AWS Console/CLI
3. ✅ Basic monitoring (CloudWatch dashboards)
4. ✅ S3 bucket for raw data + backups

**Acceptance Criteria:**
- Scoring engine processes 10,000 campaigns in <5 minutes
- Results match notebook validation dataset within 1% margin
- Error rate <0.1% on test data

#### **Phase 2: Automated Pipelines (Weeks 3-4)**
**Goal:** Scheduled data ingestion + scoring

**Deliverables:**
1. ✅ Facebook Ads API connector (Lambda)
   - OAuth refresh token management
   - Rate limit handling (200 calls/hour per account)
   - Incremental data pulls (last 7 days)
2. ✅ EventBridge scheduler (cron: 0 */6 * * *)
3. ✅ Dead letter queues (DLQ) for failed jobs
4. ✅ CloudWatch alarms for pipeline failures

**Acceptance Criteria:**
- Successful data pulls 95% of the time
- Failed jobs auto-retry 3x with exponential backoff
- Alert triggered within 5 minutes of critical failure

#### **Phase 3: User-Facing Layer (Weeks 5-6)**
**Goal:** Dashboard + email alerts operational

**Deliverables:**
1. ✅ Streamlit dashboard deployed to ECS Fargate
   - Campaign scorecard (sortable table)
   - Drill-down views (score breakdown)
   - Historical trend charts (Plotly)
2. ✅ Email alert system (SES + Lambda)
   - HTML templates (high/medium/low priority campaigns)
   - Digest emails (daily at 8 AM client timezone)
...
**Deliverables:**
1. ✅ API endpoint for programmatic access (FastAPI)
   - `/v1/campaigns/{id}/score` (GET)
   - `/v1/recommendations` (GET with filters)
   - Rate limiting (100 req/min per API key)
2. ✅ Comprehensive monitoring
   - Sentry for error tracking
   - Custom CloudWatch dashboards (latency, error rate, cost)
   - Weekly reports (system health + recommendations)
3. ✅ Disaster recovery plan
   - RDS automated backups (7-day retention)
   - S3 versioning enabled
   - Recovery time objective (RTO): 4 hours
4. ✅ Load testing (JMeter/Locust)
