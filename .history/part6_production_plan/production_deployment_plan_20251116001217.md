...
2. ✅ Verify data pipeline ran successfully:
   ```bash
   ssh ec2-user@<ELASTIC_IP>
   tail -50 /var/log/leadsmart/fetch.log
   tail -50 /var/log/leadsmart/scoring.log
   ```
3. ✅ Review Sentry errors (any new high-priority bugs?)
4. ✅ Confirm email alerts sent (check AWS SES console)
...
2. ✅ Analyze slow queries:
   ```bash
   sudo -u postgres psql leadsmart -c "
...
5. ✅ Rotate logs:
   ```bash
   sudo logrotate /etc/logrotate.d/leadsmart
   ```

**Monthly Tasks:**
1. ✅ Disaster recovery drill (restore database from backup)
2. ✅ Review SSH access logs (any suspicious activity?)
3. ✅ Update documentation (architecture diagrams, runbooks)
4. ✅ Capacity planning (CPU/memory trends, when to upgrade?)
5. ✅ Security scan:
   ```bash
   sudo lynis audit system
   ```

### 8.2 Common Issues & Resolution

```mermaid
graph TD
    Issue[🚨 Alert Triggered] --> Type{Alert Type?}
    
    Type -->|Data Pipeline Failed| P[Pipeline Issues]
    Type -->|High CPU >85%| C[CPU Issues]
    Type -->|Disk >85%| D[Disk Issues]
    Type -->|API Errors| A[API Issues]
    
    P --> P1{Check Logs}
    P1 -->|Rate Limit| P2[Wait 1 hour<br/>Manual retry]
    P1 -->|Auth Error| P3[Refresh FB token]
    P1 -->|Other| P4[Check Sentry<br/>Fix code]
    
    C --> C1{Which Process?}
    C1 -->|PostgreSQL| C2[Optimize queries]
    C1 -->|Python Script| C3[Kill hung process]
    C1 -->|Nginx| C4[Check logs]
    
    D --> D1[Clean old logs]
    D1 --> D2{Still >85%?}
    D2 -->|Yes| D3[Expand EBS volume]
    D2 -->|No| D4[Monitor]
    
    A --> A1{Error Rate?}
    A1 -->|>10%| A2[Rollback deployment]
    A1 -->|<10%| A3[Monitor + Fix]
...
**Root Causes:**
1. Facebook API token expired → Refresh token in Secrets Manager
2. Rate limit exceeded → Wait 1 hour, then manual retry
3. Database connection full → Check active connections, restart PostgreSQL if needed
4. Script crashed → Check logs for Python exception

**Resolution Steps:**
```bash
# 1. SSH into EC2
ssh -i ~/.ssh/leadsmart.pem ubuntu@<ELASTIC_IP>

# 2. Check logs
tail -100 /var/log/leadsmart/fetch.log
tail -100 /var/log/leadsmart/scoring.log

# 3. Check if cron job is running
ps aux | grep fetch_facebook_data

# 4. Manual retry (if safe)
cd /opt/leadsmart
python3 fetch_facebook_data.py
python3 score_campaigns.py
...
**Symptoms:** EC2 CPU at 90%+, API responses slow

**Root Causes:**
1. Scoring job running (expected) → Wait for completion
2. Slow database query → Identify and kill
3. Runaway process → Kill process
4. Need to upgrade instance size → Plan upgrade
...
# 3. Kill slow query (if needed)
sudo -u postgres psql leadsmart -c "SELECT pg_terminate_backend(PID);"

# 4. If Python script hung
ps aux | grep python
kill -9 <PID>

# 5. If consistently high CPU, upgrade instance
# Stop instance, change instance type to t3.large, start instance
```

#### **Issue 3: Disk Space >85%**
**Symptoms:** Alert triggered, risk of database corruption

**Root Causes:**
1. Old logs not rotated → Clean up
2. Database growing → Normal, but need more space
3. Temp files not cleaned → Clean /tmp
...
# 2. Clean old logs
find /var/log/leadsmart -name "*.log" -mtime +7 -delete
find /var/log/nginx -name "*.log" -mtime +7 -delete

# 3. Clean old backups (keep last 7)
find /backups -name "*.dump" -mtime +7 -delete

# 4. Vacuum PostgreSQL (reclaim space)
sudo -u postgres psql leadsmart -c "VACUUM FULL;"

# 5. If still >80%, expand EBS volume
aws ec2 modify-volume --volume-id vol-XXXXXX --size 100
# Then resize filesystem
sudo resize2fs /dev/xvda1
```

#### **Issue 4: API Returning Errors**
**Symptoms:** Users report 500 errors, Sentry shows spike in errors

**Root Causes:**
1. Recent deployment broke something → Rollback
2. Database connection pool exhausted → Restart app
3. FastAPI process crashed → Restart supervisor

**Resolution Steps:**
```bash
# 1. Check FastAPI status
sudo supervisorctl status leadsmart-api

# 2. Check recent logs
tail -100 /var/log/leadsmart/api_error.log

# 3. Restart FastAPI
sudo supervisorctl restart leadsmart-api

# 4. If errors persist, rollback code
cd /opt/leadsmart
...
# Check Sentry error rate, CloudWatch metrics
```

**Post-Rollback Actions:**
1. ✅ Notify team in Slack
2. ✅ Create postmortem document (what went wrong?)
3. ✅ Fix bug in development environment
4. ✅ Add test case to prevent recurrence
5. ✅ Plan re-deployment with fix
