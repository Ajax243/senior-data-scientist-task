...
def load_data():
    ...
    campaign_metrics['campaign_id'] = campaign_metrics['campaign_id'].astype(str)
    campaign_group['campaign_id'] = campaign_group['campaign_id'].astype(str)
    qualified_by_campaign['campaign_id'] = qualified_by_campaign['campaign_id'].astype(str)
...
if dashboard_choice == "📊 Executive Summary":
    ...
    if metrics_df.empty:
        ...
        with col_chart1:
            ...
            spend_by_campaign = metrics_df.nlargest(10, 'spend')[['campaign_id', 'spend']].sort_values('spend', ascending=False).copy()
            spend_by_campaign['campaign_id'] = spend_by_campaign['campaign_id'].astype(str)
            
            fig = go.Figure(data=[
                go.Bar(
                    x=spend_by_campaign['spend'],
                    y=spend_by_campaign['campaign_id'],
                    orientation='h',
        ...
        with col_chart2:
            ...
            leads_by_campaign = metrics_df.nlargest(10, 'num_leads')[['campaign_id', 'num_leads']].sort_values('num_leads', ascending=False).copy()
            leads_by_campaign['campaign_id'] = leads_by_campaign['campaign_id'].astype(str)
            
            fig = go.Figure(data=[
                go.Bar(
                    x=leads_by_campaign['num_leads'],
                    y=leads_by_campaign['campaign_id'],
                    orientation='h',
...
# ===== DASHBOARD 3: BUSINESS PERFORMANCE =====
    ...
    if user_performance.empty:
        ...
        with col1:
            fig = px.bar(user_performance, x='user_id', y='Total_Leads',
                        title="Total Leads by Business",
                        labels={'user_id': 'Business ID', 'Total_Leads': 'Total Leads'})
            st.plotly_chart(fig, use_container_width=True)
        
        with col2:
            fig = px.bar(user_performance, x='user_id', y='Avg_CPL',
                        title="Average CPL by Business",
                        labels={'user_id': 'Business ID', 'Avg_CPL': 'Avg CPL ($)'})
            st.plotly_chart(fig, use_container_width=True)
