...
if dashboard_choice == "📊 Executive Summary":
    ...
    if metrics_df.empty:
        ...
        with col_chart1:
            # Spend by Campaign
            spend_by_campaign = metrics_df.nlargest(10, 'spend')[['campaign_id', 'spend']]
            fig = px.bar(spend_by_campaign, x='campaign_id', y='spend', 
        ...
        with col_chart2:
            # Leads by Campaign
            leads_by_campaign = metrics_df.nlargest(10, 'num_leads')[['campaign_id', 'num_leads']]
            fig = px.bar(leads_by_campaign, x='campaign_id', y='num_leads',
...
# ===== DASHBOARD 4: COST & BUDGET ANALYSIS =====
    ...
    if group_df.empty:
        ...
        analysis_df = group_df.merge(
            metrics_df[['campaign_id', 'spend', 'cpl']],
            on='campaign_id',
            how='left'
        )
        ...
        with col3:
            ...
            analysis_df_clean = analysis_df.dropna(subset=['budget_per_lead', 'cpl'])
            
            fig = px.scatter(analysis_df_clean,
            ...
            max_val = max(analysis_df_clean['budget_per_lead'].max(), analysis_df_clean['cpl'].max())
