...
if dashboard_choice == "📊 Executive Summary":
    ...
    if metrics_df.empty:
        ...
        with col_chart1:
            ...
            spend_by_campaign = metrics_df.nlargest(10, 'spend')[['campaign_id', 'spend']].sort_values('spend', ascending=True).copy()
            spend_by_campaign['campaign_id'] = spend_by_campaign['campaign_id'].astype(str)
            fig = px.bar(spend_by_campaign, x='spend', y='campaign_id', 
                        title="Top 10 Campaigns by Spend", 
                        labels={'spend': 'Spend ($)', 'campaign_id': 'Campaign ID'},
                        orientation='h')
            fig.update_yaxes(categoryorder="array", categoryarray=spend_by_campaign['campaign_id'].tolist())
            st.plotly_chart(fig, use_container_width=True)
        
        with col_chart2:
            # Leads by Campaign - Horizontal bar chart
            leads_by_campaign = metrics_df.nlargest(10, 'num_leads')[['campaign_id', 'num_leads']].sort_values('num_leads', ascending=True).copy()
            leads_by_campaign['campaign_id'] = leads_by_campaign['campaign_id'].astype(str)
            fig = px.bar(leads_by_campaign, x='num_leads', y='campaign_id',
                        title="Top 10 Campaigns by Leads Generated",
                        labels={'num_leads': 'Leads', 'campaign_id': 'Campaign ID'},
                        orientation='h')
            fig.update_yaxes(categoryorder="array", categoryarray=leads_by_campaign['campaign_id'].tolist())
            st.plotly_chart(fig, use_container_width=True)
